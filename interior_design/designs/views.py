...
import os
import json
from .models import RegisterUser, UserDesign
...
def user_designs(request):
    ...
    user_designs = UserDesign.objects.filter(user_id=user_id)  # Fetch user designs

    # Manually construct the image URL
    design_data = []
    for design in user_designs:
        image_url = f"{settings.MEDIA_URL}{design.image_path}"  # Construct URL
        design_data.append({
            'design_name': design.design_name,
            'image_path': image_url,  # Pass the constructed URL
            'created_at': design.created_at.strftime('%B %d, %Y')
        })

    return render(request, 'history.html', {'user_designs': design_data})
...
def chatbot_api(request):
    if request.method == 'POST':
        try:
            ...
            bot_response = botreply(user_message)
