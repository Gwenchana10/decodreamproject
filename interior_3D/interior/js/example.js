...
var mainControls = function(blueprint3d) {
  ...
  function loadFloor() {
    const files = $("#loadFloor").get(0).files;
    ...
    if (files.length > 0) {
        ...
        reader.onload = function(event) {
            ...
            // Upload the file to the backend for processing
            const formData = new FormData();
            formData.append('file', file);

            fetch('http://localhost:8000/process-file', {  // Ensure the correct URL
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                ...
                // Load the floorplan JSON after processing
                fetch('http://localhost:8000/uploads/floorplan.json')  // Update path accordingly
                    .then(response => response.json())
                    .then(data => {
                        console.log(data);
